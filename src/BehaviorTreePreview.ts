...
export class BehaviorTreePreviewManager {
     ...
     */
    private readonly _readyPanels = new Set<vscode.WebviewPanel>();
    ...
    private _currentTopic: string = '/behavior_tree_log';
     ...
     */
    public async restorePanel(panel: vscode.WebviewPanel, uri: vscode.Uri) {
     ...
     */
    private attachPanel(panel: vscode.WebviewPanel, uri: vscode.Uri) {
        ...
        panel.onDidChangeViewState(() => {
            ...
            if (this._readyPanels.has(panel)) {
                ...
                vscode.workspace.openTextDocument(uri).then(
                    document => this.updateWebview(panel, document),
                    err => console.warn('[BehaviorTreePreview] Could not reopen document', err)
                );
            }
        });
        ...
        panel.webview.onDidReceiveMessage(message => {
            switch (message.type) {
                case 'ready':
                    this.handleReady(panel, uri).catch(err =>
     ...
     */
    private async handleReady(panel: vscode.WebviewPanel, uri: vscode.Uri) {
     ...
     */
    private post(panel: vscode.WebviewPanel, message: any) {
        if (this._readyPanels.has(panel)) {
            panel.webview.postMessage(message);
        }
    }

    private findNodeSmart(doc: Document, id: string): Element | null {
    ...
    private handleReparentNode(uri: vscode.Uri, childId: string, parentId: string) {
        const doc = this.getDoc(uri);
        if (!doc) return;

        let childNode = this.findNodeByUuid(doc, childId);
        if (!childNode) childNode = this.findNodeSmart(doc, childId);

        let parentNode = this.findNodeByUuid(doc, parentId);
        if (!parentNode) parentNode = this.findNodeSmart(doc, parentId);
    ...
    private handleDeleteEdge(uri: vscode.Uri, childId: string) {
        ...
        let childNode = this.findNodeByUuid(doc, childId);
        if (!childNode) childNode = this.findNodeById(doc, childId);
    ...
    private handleReorderChildren(uri: vscode.Uri, parentId: string, newOrder: string[]) {
        ...
        if (!foundParent) foundParent = this.findNodeSmart(doc, parentId);
        ...
        for (const childId of newOrder) {
            ...
            if (!el) el = this.findNodeSmart(doc, childId);
    ...
    private handleUpdateTopic(panel: vscode.WebviewPanel, newTopic: string) {
    ...
    private handleRenameNode(uri: vscode.Uri, id: string, newName: string) {
        ...
        let node = this.findNodeByUuid(doc, id);
        if (!node) node = this.findNodeById(doc, id);
    ...
    private findNodeByUuid(doc: Document, uuid: string): Element | null {
        const queue: any[] = [doc.documentElement];
        while (queue.length > 0) {
            const node = queue.shift();
            if (!node) continue;
    ...
    private uriForPanel(panel: vscode.WebviewPanel): string | undefined {
        for (const [uriStr, p] of this._previews) {
            if (p === panel) return uriStr;
        }
        return undefined;
    }
